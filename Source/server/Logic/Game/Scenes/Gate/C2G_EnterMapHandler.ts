...
export class C2G_EnterMapHandler extends AMRpcHandler<C2G_EnterMap, G2C_EnterMap>
{
    protected async Run(session: Session, request: C2G_EnterMap, response: G2C_EnterMap) {
        ...
        gateMapComponent.Scene = await SceneFactory.CreateServerScene(gateMapComponent, player.id, IdGenerater.inst.generateInstanceId(), "GateMap", SceneType.Map);
        ...
        let startSceneConfig = Tables.StartSceneConfigCategory.GetBySceneName(request.MapName);
