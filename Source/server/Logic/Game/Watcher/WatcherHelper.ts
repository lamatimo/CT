...
export class WatcherHelper {
    public static StartProcess(processId: number): ChildProcess {
        let cmd = `node --es-module-specifier-resolution=node --no-warnings dist/server/App/Program.js --apptype=Server --process=${processId} --develop=${Options.inst.develop}, --loglevel=${Options.inst.logLevel}`
